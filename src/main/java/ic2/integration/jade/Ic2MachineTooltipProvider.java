...
public enum Ic2MachineTooltipProvider
    ...
    public void appendTooltip(ITooltip tooltip, BlockAccessor accessor, IPluginConfig config) {
                ...
                if (data.getBoolean(KEY_STORAGE) && hasSink && hasSource) {
                    ...
                    tooltip.add(
                            Component.translatable(
                                    "ic2.electric.tooltip.output",
                                    ElectricalDisplay.formatPowerCompact(
                                            euPerTick, voltage, sourceAmps)));
                } else if (hasSource && !hasSink && recipePower > 0) {
                    tooltip.add(
                            ElectricalDisplay.formatPower(
                                    recipePower,
                                    voltage,
                                    (double) recipePower / Math.max(1, voltage.getVoltage())));
        ...
        if (data.getBoolean(KEY_STORAGE)
                && data.contains(KEY_REDSTONE_MODE)
                && JadeConfigHelper.redstoneMode().isVisible(details)) {
        ...
        if (data.contains(KEY_ACTIVE) && JadeConfigHelper.activeMode().isVisible(details)) {
            tooltip.add(
                    Component.translatable(
                            data.getBoolean(KEY_ACTIVE) ? "ic2.jade.working" : "ic2.jade.idle"));
        }
    }
}
