...
public class SoundClient extends Sound {
    ...
    public SoundClient(
        ...
        this.repeatInstance =
                new RepeatablePositionedSoundInstance(
                        soundEvent, soundCategory, volume, pitch, pos);
        this.onceInstance =
                new PositionedSoundInstance(soundEvent, soundCategory, volume, pitch, pos);
    }
    ...
    private boolean shouldKeepCurrentInstance(SoundInstance instance) {
        if (instance == null) {
            ...
            return this.vanillaManager.isActive(instance)
                    ? true
                    : this.playGraceTicks > 0 && this.startedSoundList.contains(instance);
        }
    }
    ...
    public boolean isPlaying() {
        return !this.isPlayingSound(this.onceInstance)
                ...
                ? this.isStarted && this.playGraceTicks > 0 && !this.startedSoundList.isEmpty()
                : true;
    }
    ...
    private void checkSoundFinished(SoundInstance instance) {
        if (instance instanceof ListenableSoundInstance listenableSoundInstance) {
            if (this.startedSoundList.contains(instance)
                    && !this.vanillaManager.isActive(instance)) {
