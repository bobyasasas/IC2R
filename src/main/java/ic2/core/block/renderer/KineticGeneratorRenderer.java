...
public class KineticGeneratorRenderer<T extends BlockEntity & IRotorProvider>
    ...
    public void render(
        ...
        if (diameter != 0) {
            ...
            float angle =
                    (windGen.getAngle() + windGen.getRotorAnimationSpeed() * 50.0F * partial)
                            % 360.0F;
            ...
            light =
                    LevelRenderer.getLightColor(
                            windGen.getLevel(), windGen.getBlockPos().relative(facing));
            model.render(
                    matrices,
                    vertexConsumers.getBuffer(RenderType.entitySolid(rotorRL)),
                    light,
                    overlay);
            matrices.popPose();
        }
