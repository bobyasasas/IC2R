...
public class UuIndex {
    ...
    public void refresh(boolean reset) {
        ...
        if (worldScanList.isEmpty()) {
            IC2.log.warn(
                    LogCategory.Uu,
                    "No UU world scan values configured in ic2-uu-scan-values.toml.");
        } else {
            IC2.log.debug(
                    LogCategory.Uu, "Loading UU world scan values from ic2-uu-scan-values.toml.");

            for (String entry : worldScanList) {
                this.parseUuEntry(entry, "world scan");
            }
        }

        for (String entry : IC2Config.balance.uuValues.predefined.get()) {
            this.parseUuEntry(entry, "predefined");
        }
    ...
    private void parseUuEntry(String entry, String category) {
        int splitPos = entry.lastIndexOf(32);
        if (splitPos > 0) {
            ...
            if (stack == null) {
                IC2.log.warn(
                        LogCategory.Uu,
                        "UU %s config: Can't find ItemStack for %s.",
                        category,
                        itemName);
            } else {
                try {
                    ...
                    IC2.log.warn(
                            LogCategory.Uu,
                            "UU %s config: Invalid value %s for %s.",
                            category,
                            valueStr,
                            itemName);
                }
