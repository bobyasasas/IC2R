...
public class DynamiteEntity extends Entity implements ItemSupplier {
    ...
    public DynamiteEntity(Level world, LivingEntity owner) {
        ...
        double motionX =
                -Math.sin(Math.toRadians(this.getYRot()))
                        * Math.cos(Math.toRadians(this.getXRot()));
        double motionZ =
                Math.cos(Math.toRadians(this.getYRot())) * Math.cos(Math.toRadians(this.getXRot()));
    ...
    public void tick() {
        ...
        if (this.fuse-- <= 0) {
            ...
            BlockHitResult blockHit =
                    world.clip(new ClipContext(start, end, Block.COLLIDER, Fluid.NONE, this));
            ...
            if (blockHit.getType() != Type.MISS) {
                ...
                double remainingDistance =
                        Math.sqrt(remainX * remainX + remainY * remainY + remainZ * remainZ);
                this.stickPos = blockHit.getBlockPos();
                if (remainingDistance > 0.0) {
                    this.setPos(
                            this.getX() + remainX - remainX / remainingDistance * 0.05,
                            this.getY() + remainY - remainY / remainingDistance * 0.05,
                            this.getZ() + remainZ - remainZ / remainingDistance * 0.05);
                }
            ...
            this.setYRot((float) Math.toDegrees(Math.atan2(motion.x, motion.z)));
            this.setXRot((float) Math.toDegrees(Math.atan2(motion.y, horizontalSpeed)));
            ...
            if (this.isInWater()) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    world.addParticle(
                            ...
                            this.getX() - motion.x * bubbleOffset,
                            this.getY() - motion.y * bubbleOffset,
                            this.getZ() - motion.z * bubbleOffset,
                            motion.x,
                            motion.y,
                            motion.z);
                }
    ...
    protected void readAdditionalSaveData(CompoundTag tag) {
        ...
        if (tag.contains("stickX")) {
            this.stickPos =
                    new BlockPos(tag.getInt("stickX"), tag.getInt("stickY"), tag.getInt("stickZ"));
        }
    }
