...
public class TileEntityCrop extends Ic2TileEntity implements ICropTile {
    ...
    public void performTick() {
        ...
        } else if (IC2.random.nextInt(100) == 0 && this.getStorageWeedEX() <= 0) {
    ...
    public void performWeedWork() {
        ...
        if (world != null) {
            BlockPos dstPos =
                    this.worldPosition.relative(Util.HORIZONTAL_DIRS[IC2.random.nextInt(4)]);
            if (world.getBlockEntity(dstPos) instanceof TileEntityCrop tileEntityCrop) {
                ...
                if (neighborCrop == null
                    ...
                    TileEntityCrop weedCrop = tileEntityCrop.transformCropBlock(Crops.weed, 0);
                    weedCrop.setStatGrowth(newGrowth);
                }
            } else if (world.isEmptyBlock(dstPos)) {
                if (debugWeedWork) {
                    IC2.log.info(
                            LogCategory.Block, "Block at %s - trying to generate grass", dstPos);
                }
    ...
    public boolean tryPlantIn(
            CropCard crop, int size, int statGr, int statGa, int statRe, int scan) {
    ...
    public int getCurrentAge() {
        return this.crop == null
                ? 0
                : this.getBlockState().getValue(this.getBlockType().getAgeProperty());
    }
    ...
    public void setCrossingBase(boolean crossingBase) {
        if (this.crop == null && this.level != null) {
                    ...
                    int dropCount =
                            (int)
                                    Math.max(
                                            0L,
                                            Math.round(
                                                    IC2.random.nextGaussian() * chance * 0.6827
                                                            + chance));
                    sum += dropCount;
            ...
            int dropCount =
                    (int)
                            Math.max(
                                    0L,
                                    Math.round(
                                            IC2.random.nextGaussian() * chance * 0.6827 + chance));
            List<ItemStack> ret =
    ...
    public void updateState() {
        ...
        Objects.requireNonNull(this.getLevel())
                .sendBlockUpdated(this.worldPosition, state, state, 2);
    }
    ...
    public TileEntityCrop transformCropBlock(CropCard crop, int age) {
        ...
        if (crop.getCropBlock() instanceof Ic2TileEntityBlock cropBlock) {
            ...
            if (!newState.is(Ic2Blocks.CROP_STICK)) {
                if (age > cropBlock.getCropMaxAge()) {
                    age = cropBlock.getCropMaxAge();
                }
    ...
    public TileEntityCrop transformCropBlock(Block cropBlock) {
        ...
        if (cropBlock instanceof Ic2TileEntityBlock ic2CropBlock) {
            ...
            if (!newState.is(Ic2Blocks.CROP_STICK)) {
                newState =
                        newState.setValue(
                                ic2CropBlock.getAgeProperty(), Integer.valueOf(this.currentAge));
            }
    ...
    private boolean attemptCrossing() {
        ...
        if (debugChance) {
            ...
            for (int i = 0; i < crops.length; i++) {
                ...
                System.out.printf(
                        "%s: %.1f%% %d%n%n",
                        crops[i].getUnlocalizedName(),
                        (currentChance - lastChance) * 100.0 / total,
                        ratios[i]);
