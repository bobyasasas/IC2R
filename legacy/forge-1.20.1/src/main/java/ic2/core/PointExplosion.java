    ...
    public void doExplosion() {
        if (!this.world.isClientSide) {
            ...
            if (IC2.envProxy.announceExplosion(
                    this.world, this.entity, position, this.power, this.igniter, 0, 1.0)) {
                ...
                for (BlockPos pos : affected) {
                    ...
                    if (!state.isAir()) {
                        BlockEntity blockEntity =
                                state.hasBlockEntity() ? this.world.getBlockEntity(pos) : null;
                        if (blockEntity instanceof TileEntityExplosive explosive) {
                            explosive.onExploded(this);
                        } else {
                            if (state.canDropFromExplosion(this.world, pos, this)
                                    && this.world.random.nextFloat() <= this.dropRate) {
                                Block.dropResources(
                                        state,
                                        this.world,
                                        pos,
