...
import ic2.core.IC2;
import ic2.core.block.SimpleCraftingInventory.InvSlotCraftingInventory;
...
public class ContainerIndustrialWorkbench extends ContainerFullInv<TileEntityIndustrialWorkbench> {
    ...
    protected final CraftingContainer craftMatrix =
            new InvSlotCraftingInventory(this.base.craftingGrid, 3) {
                @Override
    ...
    public ContainerIndustrialWorkbench(
        ...
        this.outputs[0] =
                this.addSlot(
                        new Ic2CraftingResultSlot(
                                this.player, this.craftMatrix, this.craftResult, 0, 124, 61) {
                            @Override
                            protected void checkTakeAchievements(ItemStack stack) {
                                if (IC2.sideProxy.isRendering()) {
                                    IC2.network
                                            .get(false)
                                            .sendContainerEvent(
                                                    ContainerIndustrialWorkbench.this, "craft");
                                } else {
        ...
        for (int y = 0; y < 3; y++) {
            for (int x = 0; x < 3; x++) {
                this.addSlot(
                        new SlotInvSlot(
                                tileEntity.craftingGrid, x + y * 3, 30 + x * 18, 43 + y * 18) {
                            @Override
        ...
        for (int y = 0; y < 2; y++) {
            for (int x = 0; x < 9; x++) {
                this.addSlot(
                        new SlotInvSlot(
                                tileEntity.craftingStorage, x + y * 9, 8 + x * 18, 106 + y * 18));
            }
        }
    ...
    private CraftingRecipe getRecipe(CraftingContainer inventory) {
        ...
        return server == null
                ...
                : server.getRecipeManager()
                        .getRecipeFor(RecipeType.CRAFTING, inventory, world)
                        .orElse(null);
    }
    ...
    public void slotsChanged(Container inventory) {
        ...
        if (world != null && world.getServer() != null) {
            ...
            ItemStack output =
                    recipe == null
                            ? ItemStack.EMPTY
                            : recipe.assemble(this.craftMatrix, world.registryAccess());
    ...
    protected ItemStack handlePlayerSlotShiftClick(Player player, ItemStack sourceItemStack) {
        T2<List<ItemStack>, ? extends IntCollection> changes =
                        ...
                        if (isOutput) {
                            ...
                            if (recipe != null
                                    && StackUtil.checkItemEquality(
                                            recipe.assemble(
                                                    inputInv,
                                                    this.base.getLevel().registryAccess()),
                                            start)) {
