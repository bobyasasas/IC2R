    ...
    public GuiScanner(ContainerScanner container, Inventory playerInventory, Component title) {
        ...
        this.addElement(
                new CustomButton(
                                ...
                                57,
                                background,
                                this.createEventSender(0))
                        .withEnableHandler(
                                () ->
                                        container.base.getState()
                                                        == TileEntityScanner.State.COMPLETED
                                                || container.base.getState()
                                                        == TileEntityScanner.State.TRANSFER_ERROR
                                                || container.base.getState()
                                                        == TileEntityScanner.State.FAILED)
    ...
    protected void drawForegroundLayer(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        switch (te.getState()) {
            ...
            case TRANSFER_ERROR:
                ...
                this.drawString(
                        guiGraphics, 105, 25, Util.toSiString(te.patternUu, 4) + "B UUM", 16777215);
                this.drawString(
                        guiGraphics, 105, 36, Util.toSiString(te.patternEu, 4) + "EU", 16777215);
        }
    }
    ...
    protected void renderBg(GuiGraphics guiGraphics, float delta, int mouseX, int mouseY) {
        ...
        if (scanning > 0) {
            this.drawTexturedRect(
                    guiGraphics.pose(), 30.0, 63 - scanning, 66.0, scanning, 176.0, 57 - scanning);
        }
    }
