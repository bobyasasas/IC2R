...
public class GuiNuclearReactor extends Ic2Gui<ContainerNuclearReactor> {
    ...
    public GuiNuclearReactor(
        ...
        IEnableHandler enableHandler = this.menu.base::isFluidCooled;
        this.addElement(
        ...
        this.addElement(
                TankGauge.createBorderless(this, 190, 54, container.base.getOutputTank(), false)
                        .withEnableHandler(enableHandler));
        this.addElement(
                new LinkedGauge(this, 7, 136, container.base, "heat", GaugeStyle.HeatNuclearReactor)
                        .withTooltip(
                                () -> {
                                    String heatPercent =
                                            String.format(
                                                    Locale.ROOT,
                                                    "%.2f",
                                                    this.menu.base.getGuiValue("heat") * 100.0);
                                    return Component.translatable(
                                                    "ic2.NuclearReactor.gui.info.temp", heatPercent)
                                            .getString();
                                }));
        this.addElement(
                TextLabel.create(
                        ...
                        TextProvider.of(
                                () ->
                                        this.menu.base.isFluidCooled()
                                                ? Component.translatable(
                                                                "ic2.NuclearReactor.gui.info.HUoutput",
                                                                this.menu.base.EmitHeat)
                                                        .getString()
                                                : Component.translatable(
        ...
        for (int y = 0; y < 6; y++) {
            for (int x = size; x < 9; x++) {
                this.drawTexturedRect(
                        guiGraphics.pose(), 26 + x * 18, 25 + y * 18, 16.0, 16.0, 213.0, 1.0);
            }
        }
