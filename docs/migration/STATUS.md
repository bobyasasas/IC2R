...
| P13 爆炸与电缆附加行为 | M12 | 进行中 | 炸药棒方块状态机、遥控器配对群爆、工业TNT（实体/方块/渲染/配方）、投掷/粘性炸药（引信链、PointExplosion、发射器、dynamite_sticky 配方）与 Ic2Explosion 射线引擎（Type 五型、吸收/穿越、dropRate 聚合掉落、实体累积伤害、ic2:nuke 与 reactor_explosion 伤害类型）已接入并双模式 GameTest 验收；核弹（NukeEntity/TileEntityNuke/GUI）、电击、涂色、建筑泡沫待迁移。勘误：legacy ItemRemote 无配对上限（基线与原始反编译均为无界列表），无需迁移。 |
