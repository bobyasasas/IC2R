...
| P13 爆炸与电缆附加行为 | M12 | 进行中 | 炸药棒方块状态机、遥控器配对群爆、工业TNT（实体/方块/渲染/配方）与投掷/粘性炸药（引信链、PointExplosion、发射器、dynamite_sticky 配方）已接入并双模式 GameTest 验收；核弹（Ic2Explosion 全尺寸）、电击、涂色、建筑泡沫与遥控器配对上限待迁移。 |
